 ...
 * like the Java Sequencer. Open a MidiInput by using {@link rwmidi.MidiInputDevice -createInput}, and use the {@link MidiInput-plug-Object} method
...
public class MidiInput implements Receiver {
  ...
  ArrayList<rwmidi.Plug>             plugList;
  ...
  public MidiInput(javax.sound.midi.MidiDevice dev2) throws MidiUnavailableException {
    ...
    plugList = new ArrayList<rwmidi.Plug>();
    currentMessage = new ArrayList();
  }

  protected MidiInput(rwmidi.MidiInputDevice _device) throws MidiUnavailableException {
...
//			printHex(message.getMessage());
  ...
  public void plug(final Object _object,
                   ...
                   final int channel,
                   final int value) {
    if (rwmidi.Plug.objectHasMethod(_object, _methodName)) {
      rwmidi.Plug plug = new rwmidi.Plug(_object, _methodName, channel, value);
      plugList.add(plug);
    }
  }
  ...
  public void plug(Object obj, int channel) {
    if (obj != null) {
      plug(obj, "noteOnReceived", channel, rwmidi.MidiEvent.NOTE_ON);
      plug(obj, "noteOffReceived", channel, rwmidi.MidiEvent.NOTE_OFF);
      plug(obj, "controllerChangeReceived", channel, rwmidi.MidiEvent.CONTROL_CHANGE);
      plug(obj, "programChangeReceived", -1, rwmidi.MidiEvent.PROGRAM_CHANGE);
      plug(obj, "sysexReceived", -1, rwmidi.MidiEvent.SYSEX_START);
    }
  }
