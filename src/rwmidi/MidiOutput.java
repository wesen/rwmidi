...
import static rwmidi.MidiEvent.CONTROL_CHANGE;
import static rwmidi.MidiEvent.NOTE_ON;
...
public class MidiOutput {
  ...
  MidiOutput(rwmidi.MidiOutputDevice _device) throws MidiUnavailableException {
  ...
  public int sendNoteOn(int channel, int note, int velocity) {
    ...
    try {
      msg.setMessage(NOTE_ON, channel, note, velocity);
      receiver.send(msg, -1);
      return 1;
  ...
  public int sendNoteOff(int channel, int note, int velocity) {
    ShortMessage msg = new ShortMessage();
    try {
      msg.setMessage(rwmidi.MidiEvent.NOTE_OFF, channel, note, velocity);
      receiver.send(msg, -1);
      return 1;
    } catch (InvalidMidiDataException e) {
      // TODO Auto-generated catch block
  ...
  public int sendController(int channel, int cc, int value) {
    ShortMessage msg = new ShortMessage();
    try {
      msg.setMessage(CONTROL_CHANGE, channel, cc, value);
      receiver.send(msg, -1);
      return 1;
    } catch (InvalidMidiDataException e) {
      // TODO Auto-generated catch block
  ...
  public int sendProgramChange(int value) {
    ShortMessage msg = new ShortMessage();
    try {
      msg.setMessage(rwmidi.MidiEvent.PROGRAM_CHANGE, value, -1);
      receiver.send(msg, -1);
      return 1;
    } catch (InvalidMidiDataException e) {
      // TODO Auto-generated catch block
  ...
  public int sendSysex(rwmidi.SysexMessage msg) {
